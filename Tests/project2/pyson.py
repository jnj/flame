...
#
# $Header: /cvs/projects/PyParse/pyson/pyson.py,v 1.25 2001/04/20 01:30:42 beazley Exp $
#
...
def check_cycles(p=None):
    if not p:
        for i in Productions:
            i.cychk = 0            # Already checked for cycles
            i.cyvalue = 0          # Cycle value
        error = 0
        p = Productions[1]
        c = check_cycles(p)
        p.cyterm = c
        for n,pl in Prodnames.items():
            ...
            for p in pl:
                try:
                    ...
                    term += 1
            if term == 0 and Nonterminals[p.name]:
                print "pyson: Infinite recursion detected in rule '%s'." % n
                error = 1
            if not used:
                print "pyson: Rule '%s' never used." % p.name
        return error

    # We are checking this rule for a cyle
    p.cychk = 1
    ...
    for s in p.prod:
        ...
        if not pl:  continue           # Must be a terminal. Continue
        allterminals = 0               # Got a nonterminal. So, rule is not all terminals
        # Check the non-terminal for cycles
        for x in pl:
            if x.cychk == 0:           # Haven't checked it
                c = check_cycles(x)
                if c:                 
                    x.cyterm = 1
                else:
                    x.cyterm = 0
            else:                      # Get previous cycle value
                c = x.cyvalue
            pterm += c
        terminate *= pterm

    if terminate or allterminals:
        p.cyvalue = 1
    else:
        p.cyvalue = 0
    return p.cyvalue
...
def parse(input = None, debug=0):
    ...
    while 1:
        ...
        if t == None:
            ...
            if errorcount > 5000:
                print "Too many errors have occurred. Bailing"
                raise SystemExit
