...
if len(sys.argv) < 2:
    print "Usage: python test2.py directory"
    raise SystemExit
...
if not dirname:
    print "Usage: python test2.py [-make] directory"
    raise SystemExit
...
print "**** Running tests for Project 2 ****"
...
for t in f:
	...
	if not os.path.exists("%s.exp" % name):
	   os.system("python flameparse.py %s.flm >%s.exp" % (name,name))
        ...
        os.system("python flameparse.py %s.flm >%s.out" % (name,name))
        try:
            f = open("%s.exp" % name)
            exp = f.readlines()
            f.close()
